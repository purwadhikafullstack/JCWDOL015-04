...
export class ReviewController {
  ...
  async createReview(req: Request, res: Response) {
    ...
    try {
      ...
      // Verifikasi apakah pengguna bekerja di perusahaan tersebut
      ...
      const ratings = [workCultureRating, workLifeBalanceRating, facilitiesRating, careerOpportunitiesRating];
      ...
      // Membuat ulasan baru jika user terverifikasi
      const newReview = await prisma.review.create({
        data: {
          ...
          company: { connect: { company_id: Number(company_id) } },
        },
      });
...
async getReviewsByCompany(req: Request, res: Response) {
  ...
  async getAverageRatingByCompany(req: Request, res: Response) {
    ...
    try {
      ...
      if (reviews.length === 0) {
        return res.status(200).json({ averageRating: 0, message: "Belum ada ulasan untuk perusahaan ini" });
      }
      ...
      res.status(500).json({ message: 'Gagal menghitung rata-rata rating', error });
    }
  }
}
