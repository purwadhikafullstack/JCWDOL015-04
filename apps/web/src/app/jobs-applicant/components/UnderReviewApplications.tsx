// components/UnderReviewApplications.tsx
'use client';
...
type UnderReviewApplicationsProps = {
  applicants: Applicant[];
};

const UnderReviewApplications: React.FC<UnderReviewApplicationsProps> = ({ applicants }) => {
  return (
    <Paper elevation={3} className="bg-white shadow-lg rounded-lg p-6 w-full">
      <Typography variant="h6" className="font-semibold mb-4 text-center text-gray-800">
        Under Review ({applicants.filter(applicant => !applicant.shortlisted).length})
      </Typography>
      {applicants
