  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        jobExpired_at: formData.jobExpired_at ? new Date(formData.jobExpired_at) : null,
      };
      console.log("Data to send:", dataToSend);
  
      if (jobId) {
        await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/jobs/${jobId}`, dataToSend, config);
        toast.success('Job updated successfully!');
      } else {
        await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/jobs`, dataToSend, config);
    ...
    <ProtectedRoute requiredRole="admin">
      <div className="max-w-3xl mx-auto my-10 p-8 bg-white shadow-md rounded-lg">
        <form onSubmit={handleSubmit} className="space-y-6">
          <h1 className="text-3xl font-semibold mb-6 text-gray-700">{jobId ? 'Edit Job' : 'Post a Job'}</h1>
